@if ($canGenerateContratacionLetters ?? false)
    <x-modal name="ficha-generate-contratacion" maxWidth="lg" focusable>
        <div
            class="modal-card ficha-empleados-generate-letters-modal"
            x-data="fichaGenerateContratacionModal()"
            x-on:ficha-prepare-generate-contratacion.window="prepare($event.detail)"
            x-on:open-modal.window="if ($event.detail === 'ficha-generate-contratacion') { loadTemplates(); loadFirmas(); }"
        >
            <div class="ficha-empleados-masivos-modal__header">
                <div class="ficha-empleados-masivos-modal__heading">
                    <div>
                        <h3 class="ficha-empleados-masivos-modal__title">Generar cartas de contratacion</h3>
                        <p class="ficha-empleados-masivos-modal__lead">Seleccione una o mas plantillas de contratacion. Una genera .docx; varias generan .zip.</p>
                    </div>
                </div>
                <button
                    type="button"
                    ...
                    aria-label="Cerrar"
                    x-on:click="$dispatch('close-modal', 'ficha-generate-contratacion')"
                >
            ...
            <div class="panel__body form-stack">
                ...
                <template x-if="! loading && ! errorMessage && templates.length === 0">
                    <p class="text-muted">No hay plantillas de contratacion con archivo. Subalas en el tablero Plantillas Word.</p>
                </template>
                ...
                <template x-if="! loading && firmas.length > 0">
                    <div class="form-field">
                        <label class="form-label" for="ficha-contratacion-signatory">Firmante</label>
                        <select
                            id="ficha-contratacion-signatory"
                            class="form-select"
    ...
    @once
        @push('scripts')
            <script>
                function fichaGenerateContratacionModal() {
                    return {
                        templatesUrl: '',
                        generateUrl: '',
                        firmasUrl: '',
                        ...
                        async submitGenerate() {
                            ...
                            try {
                                ...
                                window.dispatchEvent(new CustomEvent('close-modal', { detail: 'ficha-generate-contratacion' }));
