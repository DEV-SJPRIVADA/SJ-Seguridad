        ...
        <div class="app-container" style="padding-top: 0.75rem; padding-bottom: 0.75rem; display:flex; justify-content:space-between; gap:1rem; flex-wrap:wrap; align-items:flex-end;">
            ...
            <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                ...
                @if ($canManage)
                    ...
                    <a href="{{ route('comercial.matriz.services.create', $client) }}" class="btn btn--primary btn--sm">Agregar servicio</a>
                @endif
            </div>
        </div>
    </x-slot>
                                ...
                                @forelse ($services as $service)
                                    <tr>
                                        ...
                                        <td class="table-actions">
                                            @if ($canManage)
                                                <a href="{{ route('comercial.matriz.services.edit', [$client, $service]) }}" class="btn btn--secondary btn--sm">Editar</a>
                                                @if ($service->portfolio !== \App\Models\CommercialService::PORTFOLIO_INACTIVOS)
                                                    <form method="POST" action="{{ route('comercial.matriz.services.inactivate', [$client, $service]) }}" style="display:inline;" onsubmit="return confirm('¿Mover este servicio a Inactivos?');">
                                                        @csrf
