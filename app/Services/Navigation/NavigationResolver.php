            ...
            ->map(function (string $label, string $key) use ($staticModuleKeys, $user, $routeName, $requestModule, $requestBoard): ?array {
                ...
                $boardItems = collect(config('access.boards', []))
                    ->map(function (string $boardLabel, string $boardKey) use ($key, $user, $routeName, $requestModule, $requestBoard): ?array {
                        ...
                        if ($boardKey === 'desvinculaciones') {
                            if ($key !== 'gestion_humana') {
                                return null;
                            }

                            if (! $this->desvinculacionesAccess->canViewDesvinculacionesBoard($user)) {
                                return null;
                            }

                            return [
                                'label' => $boardLabel,
                                'route' => 'gestion-humana.desvinculaciones.masivos',
                                'url' => $user->defaultDesvinculacionesBoardUrl(),
                                'active' => str_starts_with((string) $routeName, 'gestion-humana.desvinculaciones.'),
                            ];
                        }
                        ...
                        if ($boardKey === 'cursos') {
                            ...
                            if (! $this->cursosAccess->canViewCursosBoard($user)) {
                                return null;
                            }
                        ...
                        if ($boardKey === 'seleccion') {
                            ...
                            if (! $this->seleccionAccess->canViewSeleccionBoard($user)) {
                                return null;
                            }

                            return [
                                'label' => $boardLabel,
                                'route' => 'gestion-humana.seleccion.dashboard',
                                'url' => $user->defaultSeleccionBoardUrl(),
                                'active' => str_starts_with((string) $routeName, 'gestion-humana.seleccion.'),
                            ];
                        }
                        ...
                        if ($boardKey === 'acreditaciones') {
                            ...
                            if (! $this->acreditacionesAccess->canViewAcreditacionesBoard($user)) {
                                return null;
                            }

                            return [
                                'label' => $boardLabel,
                                'route' => 'gestion-humana.acreditaciones.dashboard',
                                'url' => $user->defaultAcreditacionesBoardUrl(),
                                'active' => str_starts_with((string) $routeName, 'gestion-humana.acreditaciones.'),
                            ];
                        }
