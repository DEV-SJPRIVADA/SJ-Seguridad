...
class ContratacionLetterPackGeneratorService
{
    ...
    public function assertCanGenerate(EmployeeFichaEmploymentPeriod $period): void
    {
        if ($period->status !== EmployeeFichaEmploymentPeriod::STATUS_ACTIVO) {
            throw ValidationException::withMessages([
                'period' => 'Solo se pueden generar cartas de contratacion para vinculos activos.',
            ]);
        }
    }
    ...
    private function resolveTemplates(array $templateIds): Collection
    {
        ...
        if ($invalidType !== null) {
            throw ValidationException::withMessages([
                'template_ids' => 'Solo se pueden generar cartas con plantillas de tipo contratacion.',
            ]);
        }
    ...
    private function persistOutput(
        ...
        $relativePath = 'ficha-empleados/contratacion-letters/'.$period->id.'/'.basename($absolutePath);
    ...
    private function downloadFileName(PersonalRequisitionFichaEntry $entry, string $outputType): string
    {
        ...
        return sprintf('Cartas_Contratacion_%s_%s.%s', $document, $date, $outputType);
    }
